...
abstract class DBModel {
  ...
  public function json() {
      ...
      foreach ($this->_a as $att=>$t) {
          $json .= '"' . $att.'" : ';
          if (! isset($this->_v[$att])) {
              $json .= 'null ,';
          } else {
              switch ($t) {
                  case 'int' : {$json .=  $this->_v[$att] .' ,'; break;}
                     default : $json .=  '"' .  $this->_v[$att] .'" ,';
              }
          }
      }
...
/**
  ...
  public function setAttr($attr_name, $attr_val) {
    if (array_key_exists( $attr_name, $this->_a)) {
      ...
      if (is_null($this->_u) || (! in_array($attr_name,$this->_u)) ) $this->_u[] = $attr_name;
      return $this->_v[$attr_name];
    } 
...
/**
   ...
   */
	...
	exit();
      ...
      $r = new $this->_mname();
      foreach ($this->_a as $att => $t) $r->setAttr($att, $row[$att]);
    ...
    public function find ($params=null) {
        ...
        if (is_null($params)) return $this->findAll() ;
        if (!is_array($params)) return $this->findById($params);
